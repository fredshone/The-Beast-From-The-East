#######################################################################
# Rasterising weather station data SNOW DEPTH - EUROPE
# Fred Shone
# 26th March 2018
...
# Select data
selected <- c("PRCP_AMT") #"WIND_SPEED","SNOW_DEPTH","","AIR_TEMPERATURE"
...
# Load headers
headersUK <- fread("RH_Column_Headers.csv",header=F)

# Load data
dataUK <- fread("midas_rainhrly_201801-201812.txt",header=F)
...
regpoints <- fread("bboxEuroGrid_0_5_deg.csv",header=T) #Load regular point grid
#Convert to SP
coordinates(regpoints) <- ~ x + y
proj4string(regpoints) <- CRS(latlong)
...
dataUKV1$OB_TIME <- lubridate::ymd_hms(dataUKV1$OB_END_TIME, truncated = 1)
...
proj4string(dataSelectedEurope) <- CRS(latlong)
...
proj4string(dataSelectedUK) <- CRS(latlong)
...
# Trim NaNs
dataSelected <- dataSelected[complete.cases(dataSelected$PRCP_AMT),]
print(paste(nrow(dataSelected), "rows remaining after trim" ))

# Trim values over 100
dataSelected <- dataSelected[dataSelected$PRCP_AMT <= 100,]
print(paste(nrow(dataSelected), "rows remaining after trim" ))
...
while (t < endTime) { # Set to less than end time to allow for interpolation
  ...
  proj4string(dataHourSelected) <- CRS(latlong)
  proj4string(nextHourSelected) <- CRS(latlong)
  ...
  # Convert to raster
  if (t == startTime) {
    raster <- rasterFromXYZ(sample.idw[,c(1:3)], crs=latlong)
  } else {
    raster <- nextRaster
  }
  nextRaster <- rasterFromXYZ(nextSample.idw[,c(1:3)], crs=latlong )
  ...
  for (m in 1:3) {
    ...
    writePNG(array, paste0(subDir,"/","precipitation_",tString,".png"))
