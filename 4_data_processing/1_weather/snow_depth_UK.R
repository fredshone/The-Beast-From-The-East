#######################################################################
# Rasterising weather station data SNOW DEPTH
# Fred Shone
# 26th March 2018
...
# Select data
selected <- c("SNOW_DEPTH") #"WIND_SPEED","AIR_TEMPERATURE","","AIR_TEMPERATURE"
...
regpoints <- fread("bboxUKGrid_2_5_km.csv",header=T) #Load regular point grid
#Convert to SP
...
# data_df <- tbl_df(data)
# 
# id_type <- dplyr::group_by(data_df, ID_TYPE)
# id_type <- dplyr::summarize(id_type, count = n(), depth = mean(SNOW_DEPTH, na.rm = T), non_na_count = sum(!is.na(SNOW_DEPTH)))
# id_type
# 
# mdname <- dplyr::group_by(data_df, MET_DOMAIN_NAME)
# mdname <- dplyr::summarize(mdname, count = n(), depth = mean(SNOW_DEPTH, na.rm = T), non_na_count = sum(!is.na(SNOW_DEPTH)))
# mdname

# PRELIM DATA CLEAN

# Remove ID_TYPE: CLBD DCNN, ICAO. Keep WMO
data <- data[data$ID_TYPE == "WMO"]

# Remove stations that have no snow for the entire duration (all NAs)
src <- dplyr::group_by(data, ID_TYPE, SRC_ID)
src <- dplyr::summarize(src, count = n(), depth = mean(SNOW_DEPTH, na.rm = T), non_na_count = sum(!is.na(SNOW_DEPTH)))
src
filter <- src$SRC_ID[src$non_na_count>0]
filter
data <- data[data$SRC_ID %in% filter]
...
# Set anything >1cm to 1cm
dataV1$SNOW_DEPTH[dataV1$SNOW_DEPTH > 1] <- 1
...
while (t < endTime) { # Set to less than end time to allow for interpolation
  ...
  for (m in 1:4) {
    ...
    # Convert to 3d array
    array <- array(0, dim=c(nrow(tempRaster),ncol(tempRaster),2))
    array[,,1] <- 1
    
    upperLimit <- 0.6
    lowerLimit <- 0.2
    array[,,2] <- (colourArray-lowerLimit) / (upperLimit - lowerLimit)
    array[array[,,2] < lowerLimit] <- 0
    array[array[,,2] > upperLimit] <- 1
    
    #array[as.matrix(colourArray[,,1],2)<0.01] <- 0
    writePNG(array, paste0(subDir,"/","snow_depth_",tString,"-",dict[m],"-00.png"))
  }
