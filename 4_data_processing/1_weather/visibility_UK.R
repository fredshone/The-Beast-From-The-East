#######################################################################
# Rasterising weather station data VISIBILITY
# Fred Shone
# 26th March 2018
...
# Select data
selected <- c("VISIBILITY") #"WIND_SPEED","AIR_TEMPERATURE","","AIR_TEMPERATURE"
...
while (t < endTime) { # Set to less than end time to allow for interpolation
  ...
  sample.idw = idw(WEATHER~1, dataHourSelected, regpoints, idp = 5) %>% as.data.frame()
  nextSample.idw = idw(WEATHER~1, nextHourSelected, regpoints, idp = 5) %>% as.data.frame()
  ...
  for (m in 1:4) {
    ...
    # Colour array (high value = high visibility)
    colourArray <- as.array(tempRaster)

    # Convert to 3d array
    array <- array(0, dim=c(nrow(tempRaster),ncol(tempRaster),2))
    array[,,1] <- colourArray
    # array[,,2] <- -q75 + ((1-colourArray))
    # array[as.matrix(array[,,1],2)>q50] <- 0
    
    upperLimit <- 0.5
    lowerLimit <- 0
    array[,,2] <- 0.5 - ((colourArray-lowerLimit) / (upperLimit - lowerLimit))
    array[array[,,2] < lowerLimit] <- 0
    array[array[,,2] > upperLimit] <- 1
    
    writePNG(array, paste0(subDir,"/","visibility_",tString,"-",dict[m],"-00.png"))
